...
La implementación del \textit{backend} está implementada utilizando el marco de trabajo Spring y sus bibliotecas:
\begin{itemize}
 	...
 	\item Spring security: biblioteca para los servicios de autentificación y autorización de usuarios (ver sección \ref{sec:spring-security}).
\end{itemize}

\paragraph{\indent 1. Implementación de los servicios de seguridad\\}
Los servicios para autentificar y validar usuarios se implementaron siguiendo la especificación OAtuh 2.0 (ver sección \ref{sec:oauth}), el marco de trabajo Spring con sus bibliotecas \textit{Spring boot} y \textit{Spring security}.\\
...
\end{lstlisting}
	...
	\item Habilitar servicios de autorización: se refiere a dar un \textit{token} de acceso (como lo especifica OAuth 2.0), en el código \ref{lst:enable-auth-server} se muestra la declaración de una clase para la configuración de un servidor de autorización:
...
\end{lstlisting}
	...
	\begin{enumerate}
		\item Línea 2: inyección del \textit{bean}\footnote{En Spring un bean es un objeto en el contexto de la aplicación\cite{SpringInAction}} que realiza la autentificación de usuarios.
...
\end{lstlisting}
	...
	\begin{enumerate}
		...
		\item Línea 7: enunciación de las URL públicas.
		\item Línea 8: enunciación de las URL que requieren de un usuarios autorizado. 
	\end{enumerate}
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item Línea 3: inyección del \textit{bean} de MyBatis para administrar órdenes de reposición.
	\end{enumerate}
...
\end{lstlisting}
	...
	Dentro de la clase declara como un \textit{bean} que exponen servicios web se declaran métodos que serán expuestos como un servicio web. En el Código \ref{lst:get-orden-data-controller} se muestra el servicio web para obtener una orden de reposición:
	\begin{enumerate}
...
\end{lstlisting}
	...
	\begin{enumerate}
		...
		\item Línea 16: si el reporte no es vacío, entonces se manda el reporte como flujo de \texttt{bytes}.
...
La implementación del \textit{frontend} está basada en el marco de trabajo \textit{AngularJS} (ver sección \ref{sec:angularjs}), en el Código \ref{lst:portal-js} como se muestra la implementación de la aplicación con \textit{AngularJS}:
\begin{enumerate}
	\item Línea 1: creación del módulo de \textit{AngularJS}.
	\item Líneas 2 a 19: configuración de las vistas y rutas.
\end{enumerate}
\begin{lstlisting}[language=Javascript, caption={Módulo de \textit{AngularJS} para el portalWeb}, captionpos=b, label={lst:portal-js}]
...
A continuación se hace una descripción detalladas de la implementación de las vistas que ofrece el \textit{frontend}:

\paragraph{\indent 1. Vista Acceso\\}
Los flujos de autentificación y autorización del sistema AutoSA se hacen siguiendo la especificación de OAuth (ver sección \ref{sec:oauth}). La autentificación se muestra al usuario mediante la plantilla del Código \ref{lst:longin-view} donde se ligan el nombre de usuario y la contraseña con el modelo de angular.
...
\begin{lstlisting}[language=Javascript, caption={Uso del servicio que optine un \textit{token} de acceso.}, captionpos=b, label={lst:login-ctrl-js}]
...
\begin{enumerate}
	...
	\item Línea 3: mapa de configuración para la llamada al servicio Web de \textit{token} de acceso, se muestran únicamente las propiedades más relevantes.
...
La plantilla HTML tiene una forma como elemento principal, en el Código \ref{lst:view-report-form} se muestra la declaración de la forma y el botón para enviar el formulario, el botón está ligado a la función \texttt{generate} del controlador \texttt{reportesCtrl}:
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item La directiva \texttt{ng-disabled} estable que el botón será habilitado cuando se cumpla la expresión que contiene, en este caso, que la forma tenga datos válidos.
	\end{enumerate}
\end{enumerate}
...
La vista está relacionada con el control \texttt{catalogosCtrl}, por su parte el botón para actualizar el catálogo seleccionado está ligado a la función \texttt{update}, en el Código \ref{lst:catalogo-ctrl-js} se muestra la implementación del control de la vista:
...
Si bien la viste tiene dos componentes principales que se encargan de buscar y mostrar órdenes de reposición, el controlador \texttt{busquedaCtrl} ofrece la funcionalidad a esos componentes mediante las operaciones:
\begin{itemize}
