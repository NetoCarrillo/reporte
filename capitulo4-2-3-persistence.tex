...
\begin{enumerate}
 	\item Escritorio: la automatización de rutinas, dado que éstas son ejecutadas dentro del ambiente de \textit{Sahi} (que a su vez está en la plataforma de \textit{Java}) se utiliza la biblioteca \textit{JDBC} para economizar los recursos físicos del equipo del operador de la farmacéutica.
...
La clase \texttt{IResultSetMapper} tiene como función la conversión de la información obtenida por la biblioteca \textit{JDBC} de la base de datos a objetos del dominio del proyecto AutoSA. El Código \ref{lst:per-i-rs-mapper} muestra la definición de la interfaz de \textit{Java}.
...
El Código \ref{lst:per-class-mapper-provider} muestra la declaración del la clase \textbf{ResultSetMapperProvider} donde se observa el uso del patrón de diseño \textit{Singleton}:
\begin{enumerate}
...
\begin{lstlisting}[language=Java, caption={Clase ResultSetMapperProvider con patrón de diseño \textit{Singleton}.}, captionpos=b, label={lst:per-class-mapper-provider}]
...
\begin{enumerate}
	...
	{\setlength\itemindent{25pt}\item Consulta \textit{SQL} para obtener un usuario.}
\end{enumerate}
